这章节从头到尾就是讲了各种从底层视角的加速方法，读起来真的太坐牢了（主要是我感受不到这部分的加速效率），而且这些造轮子的工作其实早就由架构师做完了，现在重学一遍我感觉这张其实不适合大家 速成 HPC，而更多的是一种拓展的视角
...
乱序执行要求我们的程序按照程序开始的顺序来执行，这种执行方式和我们最开始讲到的等待方法相比实现了寄存器的重命名，不同线程实现寄存器的动态调整，来实现并行计算（应该是这个意思？可以看下原文）

![](../../images/2026-07-14-15-26-54.png)

### The complexity of x86 ISA

x86架构是复杂的，因为x86属于CISC，其一是因为他指令长度不固定，其二是由于他编码方式复杂，其三是由于指令的抽象性还是有点高（可以同时包含访存和计算）

为了解决这种复杂性，CPU中的Front End 会做一种改动——他会把复杂的x86字节流转化成一种更容易执行的micro-ops（我们之前看到的取址到执行的过程）

通过这样（当然还有很多其他的技术）的优化，我们得到了一个Hsawell微架构

![](../../images/2026-07-14-15-39-32.png)
![](../../images/2026-07-14-15-40-42.png)

### Simultaneous Multithreading

同时多线程技术是通过用一个物理核心来维护多个硬件线程，比如同时给每个核心分配两个线程，在OS中这种分配会直接被识别成两个logical cpu，当然，他们并斌不是两个完整的核心

这种方式能够有效利用闲置的算力资源，比如线程A遇到cache miss的时候B能够跑点别的东西

但是在HPC的场景中，我们的这种技术反而可能降低性能（计算密集程序吧计算单元都抢了、Cache被抢了、带宽被抢了……）所以这只是一种提高算例利用率的方法，在实际场景中的加速效果还要因机而异

## Single Instruction Multiple Data

上面我们从x86架构的角度实现了计算的并行化，那么从指令曾main我们能不能优化呢，当然可以

![](../../images/2026-07-14-15-58-03.png)

SIMD通过一个2656位的寄存器实现加速（拆成四个64位的就寄存器），这种加速方案实现了四个数据的并行计算，从而实现了计算的加速

![](../../images/2026-07-14-16-00-24.png)

当然，根据原理我们可以知道这种并行方法需要我们的寄存器做计算支持，从上面这张图中我们也可以看见几种划分寄存器的方法

### control divergence

控制流分歧是让同一条SIMD指令给所有数据执行相同的操作，当然，不同元素的条件可能不同
...
SIMD 一次处理多个 A[i]，其中可能有些大于 0、有些不大于 0。CPU 通常先向量化比较，得到一个 mask：
...
一种方法是推测式地对所有通道计算 B[i] + C[i]，然后用 mask 只保留条件成立通道的结果，问题是，被 mask 丢弃的结果虽然不会写回，但对应操作未必都可以安全执行
...
在这种情况下，如果无条件对所有通道读取 A[i]，越界通道可能访问非法地址。除法、开方等操作也可能产生异常或无效值，所以，分支简单且内存访问安全时，mask SIMD 通常有效；分支复杂或可能越界时，标量处理或重新组织数据可能更合适（大概是这个意思）

### blend

![](../../images/2026-07-14-16-39-00.png)

blend 和上面讲的其实差不都，本质上都是通过我们的mask来进行新向量的构建
...
这一部分讲的是从cpu角度实现我们的计算优化

但是我看了一下我们之前都说过了

L1 L2独享 L3作为内存池 多核并行等等技术
...
我们之前讲过，在单核的时候我们可以通过valid和dirty来判断我们的缓存是否更改，但是当上升到多核的时候，这种情况就变了，一个地址当然可以存放在很多寄存器中，所以我们需要一种新的协议来判断 别的地址有没有这个内容

下面是MSI的具体定义方式

![](../../images/2026-07-14-20-23-32.png)