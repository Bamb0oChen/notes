...
直到ios6 苹果的工程师才映入虚拟内存映射和ASLR(address space layout randomization)技术（内存页面偏移技术）
...
在ios12中，工程师们想到了一种新的方法：PAC(Pointer Authentication Codes)

但是我们现在的芯片实现了一种新的优化，很多黑客会尝试通过修改返回指针来执行侵入性程序，PAC的出现让ROP(导向编程)不再可行，所有的地址都经过加密签名，签名信息储存在指针的高位，当跳转到新的地址的时候ARM64E指令会执行认证逻辑，确保签名在当前环境下仍然有效，替换返回地址将触发PAC异常
