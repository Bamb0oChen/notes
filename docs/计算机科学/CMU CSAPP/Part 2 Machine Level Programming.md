...
这是一张栈的表示图，请记住他，我们的栈基本都是从上往下写，栈底在顶上喔，而且越往栈顶地址越小



![](../../images/2026-09-13-14-52-09.png)

对，这张图已经把**递归、寄存器保存、栈帧和函数返回**串起来了。最关键的一点是：**递归的每一层都使用同一套寄存器，靠栈保存和恢复之前的状态。**
...
这一段没有 `subq` 分配大块局部空间，也没有用 `%rbp`，但仍然存在每次调用对应的栈上记录。

执行到最深层 `pcount(0)` 时，栈可以简化成：
...
**所以递归并不是每调用一次就多出一套寄存器，而是每深入一层，就在栈里留下恢复现场所需的信息；返回时再逐层取回来。**